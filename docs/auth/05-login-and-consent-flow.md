...
描述學員經登入頁進站、以及條款同意的完整產品流程，涵蓋兩條路徑：

**登入頁路徑**（§3 流程圖）

- 登入頁顯示提示：請使用報名 email 登入 Google
- 登入頁勾選同意條款（互惠條款 + 隱私政策），條款以彈窗預覽
- Google SSO，比對 email 與學員白名單
- 首次登入：建立學員資料，並寫入同意紀錄
- 再次登入：不重建學員資料，同版本的同意紀錄不重複新增（§8.1）
- 不在白名單：登入頁拒絕狀態（含申訴入口）

**非登入頁路徑**

- 條款版本更新後的重新同意（§6）：使用者已持有 session、不會再經過登入頁，改以逐請求檢查版本並導向重簽頁
...
| 0.10 | 2026-08-22 | **檔名與標題對齊實際範圍**（PR review 回饋：原本只指涉首次登入，但內容含再次登入與條款改版重簽）：檔名改為 `05-login-and-consent-flow.md`、標題改為「學員登入與條款同意流程」；§1 重寫為登入頁路徑（首次／再次登入）與非登入頁路徑（§6 重簽）兩條；§3 標題改為「登入頁流程圖」。內容邏輯未變 |
