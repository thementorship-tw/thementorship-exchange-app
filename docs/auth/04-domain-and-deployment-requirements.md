...
> **對象：** 負責 DNS、部署、網域的同事
> **使用階段：** Phase 0～1（決定 URL）；Phase 3 前需就緒（prod 驗收）
> **提出者：** Tech Lead
...
環境清單以 **01-overview §4** 為準；本節說明各環境對 Infra 的需求。
...
| 環境 | 建議子網域 | 備註 |
|------|-----------|------|
| Production | `app.<product-domain>` 或根網域 | 例：`app.thementorship.exchange`；團隊可共識 |
...
- Production 的 OAuth 設定可等 URL 確定後再追加
...
| # | 交付項目 | 範例 |
|---|----------|------|
| 1 | Production Base URL | `https://app.example.com` |
| 2 | 部署平台 | Vercel / GCP / 其他 |
| 3 | TLS 是否就緒 | 是 / 否 / 預計日期 |
| 4 | 環境變數注入方式 | Vercel Env / GCP Secret Manager / 其他 |
